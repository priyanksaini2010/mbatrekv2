...
class Calendar {
	...
	if (!empty($data['sessions'])) {
	    ...
	    $cellContent = $cellContent."<span class='events'> ".$data['sessions']->session_name."(".date("h:i a",strtotime($data['sessions']->session_date." ".$data['sessions']->session_time)).")"."</span>";
//	    pr($cellContent);
            $class = "data_active ";
	}
	return '<li id="li-' . $this->currentDate . '" class="'.$class . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
