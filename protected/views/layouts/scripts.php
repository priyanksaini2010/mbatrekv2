...
//	$("input[name='example10']").change(function(){
//	    alert($(this).val());
//	});
	$("#example10_proj").dateDropdowns({
      ...
      //  $('.menu_list').slideAndSwipe();
	    ...
	    if ($("#Users_cpassword").val() == "") {
                $("#Users_cpassword").focus()
                $('#myModalcpassword').modal('show');
                return false;
            }
	    if ($("#Users_cpassword").val() != $("#Users_password").val() ) {
                $("#Users_cpassword").focus()
                $('#myModalcpassworde').modal('show');
                return false;
            }
            if ($("#Users_fname").val() == "") {
                $("#Users_fname").focus()
	    ...
	    if ($("#institute_id").val() == "") {
                $("#institute_id").focus()
                $('#myinstitute_idname').modal('show');
                return false;
            }
	  
	    if ($("#institute_id").val() == 0) {
		if ($("#Users_subscription").val() == 0) {
		    $("#Users_subscription").focus()
		    $('#myUsers_subscription').modal('show');
		    return false;
		}
	    }
	  
            if ($("#Users_program").val() == "") {
                $("#Users_program").focus()
                $('#myUsers_program').modal('show');
                return false;
            }
            
            if ($("#Users_batch").val() == "") {
                $("#Users_batch").focus()
                $('#myUsers_batch').modal('show');
                return false;
            }
        });
        //Hand Book
        ...
        $(".remark").click(function() {
                ...
                data : {
                    ...
                    rating : $(this).val(),
		    rating_type : $(this).attr("rating_type")
                },
                success: function(data) {
                    if (data == "ok") {
...
//            }
	...
	$(".sub_pre_session").click(function(){
	   
	    var rel = $(this).attr('rel');
	    var jsonobj = {
                    'mentor': $("#mentor"+rel).val(),
                    'prior_understanding': $("#prior_understanding"+rel).val(),
                    'prior_experience': $("#prior_experience"+rel).val(),
                    'expectation': $("#expectation"+rel).val(),
                    'current_status': $("#current_status"+rel).val(),
                    'institute_batch_notification_id': $("#institute_batch_notification_id"+rel).val(),
                   
                };
if (isEmpty(jsonobj.mentor) || isEmpty(jsonobj.prior_understanding) || isEmpty(jsonobj.prior_experience) || isEmpty(jsonobj.expectation) || isEmpty(jsonobj.current_status)) {
		   alert("Please fill all fields in form."); 
		   return false;
		}
		
	     $.ajax({
		url: '<?php echo Yii::app()->createUrl('student/saveevaluation'); ?>',
                type: 'post',
		data: jsonobj,
		success: function(data) {
		    var decodedData = $.parseJSON(data);
		    if (decodedData.status == "success") {
			$("#myModalnot"+rel).modal('hide');
			$(".body_class_identifier").removeClass("model_for_pree_session");
		    } else {
			
			$("#myModalnoterror").modal('show');
		    }
		}
	     });
	});
	$("#submit_add").click(function(){
	    $("#filter-form-adddoc").submit();
	});
	$(".sub_post_session").click(function(){
	   
	    var rel = $(this).attr('rel');
	    var jsonobj = {
                    'mentor': $("#mentor"+rel).val(),
                    'Agreement_with_session_and_mentor': $("#Agreement_with_session_and_mentor"+rel).val(),
                    'Expectations_met': $("#Expectations_met"+rel).val(),
                    'Learning_from_colleagues': $("#Learning_from_colleagues"+rel).val(),
                    'Real_situation_applicability': $("#Real_situation_applicability"+rel).val(),
                    'rating': $("#rating"+rel).val(),
                    'institute_batch_notification_id': $("#institute_batch_notification_id"+rel).val(),
                   
                };
if (isEmpty(jsonobj.mentor) || isEmpty(jsonobj.Agreement_with_session_and_mentor) || isEmpty(jsonobj.Expectations_met) || isEmpty(jsonobj.Learning_from_colleagues) || isEmpty(jsonobj.Real_situation_applicability) || isEmpty(jsonobj.rating)) {
		   alert("Please fill all fields in form."); 
		   console.log(jsonobj);
		   return false;
		}
	     ...
	     $.ajax({
		url: '<?php echo Yii::app()->createUrl('student/saveevaluationpost'); ?>',
                type: 'post',
		data: jsonobj,
		success: function(data) {
		    var decodedData = $.parseJSON(data);
	...
	$(".traits").click(function(){
	     $('#myModalTraits').modal('show');
	});
        $("#submit_library_profile_ind").click(function() {
            $("#filter-form-profile-ins").submit();
        })
	    ...
	    if ($("#LoginForm_new_username").val() != $("#Users_cpassword").val() ) {
	       $("#Users_password").focus()
	       $('#myModalcpassworde').modal('show');
	       return false;
	    }
...
//        Institute Profile Popups
$("#filter-form-profile-ins").submit(function() {
            if ($("#fname").val() == "") {
                $("#fname").focus()
                $('#myModalfname').modal('show');
                return false;
...
<?php } ?>
<?php if (isset($_REQUEST['thankdoc']) && $_REQUEST['thankdoc'] == 1) { ?>

            $("#myModalthankdoc").modal('show');
...
<?php } ?>
<?php if (isset($_REQUEST['thankpass']) && $_REQUEST['thankpass'] == 1) { ?>

        $("#myModalthankpassword").modal('show');

<?php } ?>
<?php if (isset($_REQUEST['thankpassfail']) && $_REQUEST['thankpassfail'] == 1) { ?>

        $("#myModalthankpasswordfail").modal('show');
...
<?php } ?>
    $("#open-position").click(function(){
	 $("#myModalopenposition").modal('show');
    })
...
foreach ($studentData->instituteBatch->instituteBatchNotifications as $notifictaion) {
    if($notifictaion->status != 1){continue;} 
    $isShown = InstituteBatchNotificationStudent::model()->findByAttributes(array('student_id' => $studentData->id,'institute_batch_notification_id' => $notifictaion->id));
   
    $isShownPost = InstituteBatchNotificationStudentPost::model()->findByAttributes(array('student_id' => $studentData->id,'institute_batch_notification_id' => $notifictaion->id));
    if (count($isShown) == 0 && $notifictaion->type == 1) {
?>
	$("#myModalnot"+<?php echo $notifictaion->id;?>).modal('show');
	$(".body_class_identifier").addClass("model_for_pree_session");
	$(".body_class_identifier").removeClass("remove_overflow");
<?php 
	
    }
    if(count($isShownPost) == 0 && $notifictaion->type == 2) {?>
...
<script>
		$(document).ready(function(){
			...
			$("#addCF").click(function(){
				...
				$("#customFields").append('<tr><td><input type="text" name="exam[]"></td><td><input type="text" name="school[]"></td><td><input type="text" name="board[]"></td><td><input type="text" name="year[]"></td><td><input type="text" name="cgpa[]"></td><td><div class="sibmit_form"><div class="site_btn"><button class="raised ripple has-ripple " id="remCF" type="button">Remove</button></div></div></td></tr>');
			});
	...
	<script>
	$("#myModalnot9").on('hidden.bs.modal',function(){
