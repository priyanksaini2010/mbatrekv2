...
class CollegesCompetitionController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['CollegesCompetition']))
		{
			$model->attributes=$_POST['CollegesCompetition'];
			if($model->save()){
	...
	{
		$all = CollegesCompetition::model()->findAll();
		foreach ($all as $item){
		    if($item->id != 4){
	...
	{
		$dataProvider=new CActiveDataProvider('CollegesCompetition');
	...
	{
		$model=new CollegesCompetition('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['CollegesCompetition']))
			$model->attributes=$_GET['CollegesCompetition'];
	...
	{
		$model=CollegesCompetition::model()->findByPk($id);
		if($model===null)
